...
export class AdminService {
  ...
  async loginAdmin({ email, pass }) {
    ...
    if (
      !user?.roles?.includes(ERoles.ROOT) &&
      !user.roles.includes(ERoles.MANAGER)
    )
  ...
  async createAdmin(stream: Readable, input: ICreateAdminDTO) {
  ...
  async editAdmin({
    ...
    if (password)
      fieldsPendingUpdate.pass = await this.authService.encryptString(password);
